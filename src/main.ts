...
async function run() {
  try {
    ...
    const componentsThreshold = new Map<Component, StatusCode | undefined>([
      ...
      [Component.DockerDesktop, await getStatus('desktop_threshold')],
      [Component.DockerBilling, await getStatus('billing_threshold')],
      ...
      [Component.DockerCloud, await getStatus('cloud_threshold')],
      [Component.DockerOffload, await getStatus('offload_threshold')],
      [Component.DockerHardenedImages, await getStatus('hardened_images_threshold')]
    ]);
